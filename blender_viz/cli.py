...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--opaque-background", action="store_true", help="render the arena floor and black world instead of transparent RGBA")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not mjcf.exists():
        available = ", ".join(p.stem.removeprefix("racing_simple_") for p in sorted((ROOT / "envs/mjx").glob("racing_simple_*.xml")))
    ...
    try: width, height = (int(v) for v in args.resolution.lower().split("x", 1))
    except ValueError: raise SystemExit("--resolution must look like 1920x1080")
    spec = {"trajectory": trajectory, "gates": gates, "track": args.track,
            "output": str(args.output.resolve()), "blend": str(args.blend.resolve()) if args.blend else None,
            ...
            "transparent": not args.opaque_background, "engine": args.engine,
            "samples": args.samples, "width": width, "height": height, "fps": args.fps,
            "animation": args.animation, "camera": args.camera, "open_ui": args.open_ui}
    if args.dry_run:
        summary = {**spec, "trajectory": {k: (len(v) if isinstance(v, list) else v) for k, v in trajectory.items()}}
        print(json.dumps(summary, indent=2)); return
    blender = find_blender(args.blender)
    if not blender: raise SystemExit("Blender was not found. Install Blender 4.x or pass --blender /path/to/blender. Use --dry-run to validate without it.")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    if args.blend: args.blend.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as handle:
        json.dump(spec, handle); spec_path = Path(handle.name)
    script = Path(__file__).with_name("scene.py")
    command = [blender]
    if not args.open_ui: command.append("--background")
    ...
    finally: spec_path.unlink(missing_ok=True)
