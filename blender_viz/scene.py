...
def materials():
    M.update(
        {
            "carbon": mat("Carbon fiber", (0.012, 0.016, 0.025), metallic=0.72, roughness=0.22),
            "metal": mat("Anodized metal", (0.06, 0.085, 0.13), metallic=0.9, roughness=0.18),
            "white": mat("Gate ceramic", (0.55, 0.65, 0.78), metallic=0.35, roughness=0.23),
            "cyan": mat("Electric cyan", (0.005, 0.32, 0.72), emission=(0.005, 0.35, 1), strength=10),
            "blue": mat("Velocity blue", (0.005, 0.08, 0.8), emission=(0.005, 0.06, 1), strength=14),
            "violet": mat("Velocity violet", (0.35, 0.015, 0.75), emission=(0.45, 0.01, 1), strength=14),
            "pink": mat("Velocity magenta", (1, 0.015, 0.18), emission=(1, 0.005, 0.08), strength=16),
            "amber": mat("Gate amber", (1, 0.12, 0.008), emission=(1, 0.05, 0.002), strength=12),
            "glass": mat("Propeller glass", (0.015, 0.15, 0.32), metallic=0.15, roughness=0.08, alpha=0.42),
            "ground": mat("Arena floor", (0.008, 0.012, 0.022), metallic=0.25, roughness=0.28),
        }
    )
...
def curve(name, points, material, bevel=0.025, cyclic=False):
...
def create_gate(gate, index):
    ...
    root = bpy.data.objects.new(f"Gate {index + 1:02d}", None)
    bpy.context.collection.objects.link(root)
    root.location = gate["position"]
    root.rotation_euler = gate["rotation"]
    if GATE_PROTOTYPES is None:
        ...
        if texture_path and Path(texture_path).exists():
            cover = bpy.data.materials.new("Unbranded printed gate fabric")
            ...
            for obj in GATE_PROTOTYPES:
                if obj and obj.name.startswith("Cover"):
                    ...
                    obj.data.materials.append(cover)
    holder = bpy.data.objects.new(f"Gate {index + 1} asset transform", None)
    ...
    for prototype in GATE_PROTOTYPES:
        ...
        obj.name = f"Gate {index + 1} {prototype.name}"
        bpy.context.collection.objects.link(obj)
        obj.parent = holder
    return root
...
def create_drone(name="Racing drone", scale=1.0):
    ...
    body = cube("Flight controller", (0, 0, 0), (0.12, 0.09, 0.035), M["carbon"], 0.025)
    body.parent = root
    canopy = uv_sphere("Camera canopy", (0.07, 0, 0.045), 0.07, M["metal"])
    canopy.scale = (1.25, 0.8, 0.65)
    canopy.parent = root
    # X-frame arms, motors, luminous prop discs.
    for i, (x, y) in enumerate(((0.18, 0.18), (0.18, -0.18), (-0.18, 0.18), (-0.18, -0.18))):
        arm = cylinder_between("Carbon arm", (0, 0, 0), (x, y, 0), 0.018, M["carbon"], 12)
        arm.parent = root
        motor = cylinder_between("Motor", (x, y, -0.012), (x, y, 0.055), 0.032, M["metal"], 20)
        motor.parent = root
        bpy.ops.mesh.primitive_cylinder_add(vertices=48, radius=0.105, depth=0.004, location=(x, y, 0.062))
        prop = bpy.context.object
        prop.name = f"Motion-blurred propeller {i + 1}"
        prop.data.materials.append(M["glass"])
        prop.parent = root
        hub = uv_sphere("Prop hub", (x, y, 0.067), 0.022, M["cyan"])
        hub.parent = root
    # forward-facing camera lens and rear status light
    camera = cylinder_between("FPV camera", (0.125, 0, 0.035), (0.17, 0, 0.035), 0.03, M["carbon"], 24)
    camera.parent = root
    lens = uv_sphere("FPV lens", (0.174, 0, 0.035), 0.022, M["cyan"])
    lens.scale.x = 0.35
    lens.parent = root
    tail = uv_sphere("Tail light", (-0.13, 0, 0.035), 0.025, M["pink"])
    tail.parent = root
...
def compute_speeds(points):
    ...
    if supplied:
        return [float(v) for v in supplied]
    speeds = [0.0]
    for a, b in zip(points, points[1:]):
        speeds.append((Vector(b) - Vector(a)).length)
    if len(speeds) > 2:
        speeds[0] = speeds[1]
    return speeds
...
def create_trail(points):
    ...
    curve("Trajectory aura", points, M["cyan"], 0.045)
    ...
    for i in range(len(points) - 1):
        ...
        if key != current or i == len(points) - 2:
            ...
            if end > start:
                curve(f"Speed trail {start:04d}", points[start : end + 1], M[current], 0.023)
            start, current = i, key
    ...
    for i in range(stride, len(points) - 1, stride):
        ...
        bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=0.09, radius2=0, depth=0.24, location=points[i])
        arrow = bpy.context.object
        arrow.name = "Direction chevron"
        arrow.rotation_mode = "QUATERNION"
        arrow.rotation_quaternion = tangent.to_track_quat("Z", "Y")
        arrow.data.materials.append(M["amber"])
...
def tangent_quaternion(points, i):
    ...
    if 2 <= i < len(points) - 2:
        a = (Vector(points[i]) - Vector(points[i - 2])).normalized()
        b = (Vector(points[i + 2]) - Vector(points[i])).normalized()
        bank = max(-0.65, min(0.65, a.cross(b).z * 1.8))
        q = q @ Quaternion((1, 0, 0), bank)
    return q


def animate_drone(drone, points):
    n = len(points)
    frames = max(72, min(360, n))
    bpy.context.scene.frame_end = frames
    quats = SPEC["trajectory"].get("quaternions")
    for frame in range(1, frames + 1):
        i = round((frame - 1) / (frames - 1) * (n - 1))
        drone.location = points[i]
    ...
    if drone.animation_data and hasattr(drone.animation_data.action, "fcurves"):
        for curve_ in drone.animation_data.action.fcurves:
            for key in curve_.keyframe_points:
...
def setup_world(points):
    ...
    try:
        ...
        if SPEC["engine"] == "BLENDER_EEVEE_NEXT":
    ...
    extent = max(max(xs) - min(xs), max(ys) - min(ys), 5)
    if not SPEC.get("transparent", True):
        floor = cube(
            "Reflective arena floor",
            ((max(xs) + min(xs)) / 2, (max(ys) + min(ys)) / 2, -0.12),
            (extent * 0.9, extent * 0.9, 0.08),
            M["ground"],
            0.08,
        )
        for radius in (extent * 0.38, extent * 0.62, extent * 0.82):
            ring = []
            for i in range(97):
                a = 2 * math.pi * i / 96
                ring.append((floor.location.x + radius * math.cos(a), floor.location.y + radius * math.sin(a), -0.025))
            curve("Arena light ring", ring, M["cyan"], 0.009, True)
    # Area key lights and low colored rim lights.
    center = Vector((sum(xs) / len(xs), sum(ys) / len(ys), sum(p[2] for p in points) / len(points)))
    for idx, (loc, color, energy, size) in enumerate(
        (
            ((center.x - extent, center.y - extent, 7), (0.12, 0.3, 1), 1300, 6),
            ((center.x + extent, center.y + extent, 5), (1, 0.03, 0.18), 900, 4),
        )
    ):
        data = bpy.data.lights.new(f"Arena light {idx}", "AREA")
        data.energy = energy
        data.color = color
        data.shape = "DISK"
        data.size = size
...
def camera_for(points, drone):
    ...
    for gate in SPEC["gates"]:
        ...
        bounds.extend(
            (p + Vector((0, 0, 1.25)), p - Vector((0, 0, 1.25)), p + Vector((0, 1.0, 0)), p - Vector((0, 1.0, 0)))
        )
    ...
    cam = bpy.data.objects.new("Cinematic camera", data)
    bpy.context.collection.objects.link(cam)
    scene.camera = cam
    data.lens = 48
    mode = SPEC["camera"]
    if mode == "top":
        cam.location = (center.x, center.y, maxs.z + radius * 2.65)
        data.lens = 52
    else:
...
def main():
    ...
    for datablocks in (bpy.data.curves, bpy.data.meshes, bpy.data.materials, bpy.data.cameras, bpy.data.lights):
        pass
    materials()
    points = SPEC["trajectory"]["positions"]
    setup_world(points)
    create_trail(points)
    ...
    flight_rig = bpy.data.objects.new("Invisible flight path rig", None)
    bpy.context.collection.objects.link(flight_rig)
    flight_rig.rotation_mode = "QUATERNION"
    animate_drone(flight_rig, points)
    camera_for(points, flight_rig)
    scene = bpy.context.scene
    scene.render.filepath = SPEC["output"]
    scene.render.fps = SPEC["fps"]
    if SPEC.get("blend"):
        bpy.ops.wm.save_as_mainfile(filepath=SPEC["blend"])
    if SPEC["open_ui"]:
        return
    if SPEC["animation"]:
        ...
        scene.frame_set(round(scene.frame_end * 0.62))
